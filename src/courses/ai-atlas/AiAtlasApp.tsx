...
import { getProfessorAudios } from '../../professorAudio'
import { courseDocumentHash, parseCourseRoute } from '../../courseRouting'
import { AmbientBackground } from '../../components/AmbientBackground'
import { AudioSummary } from '../../components/AudioSummary'
import { HomeCover } from '../../components/HomeCover'
import { MarkdownArticle } from '../../components/MarkdownArticle'
import { PodcastGuide } from '../../components/PodcastGuide'
import { SearchDialog } from '../../components/SearchDialog'
import { Sidebar } from '../../components/Sidebar'
import { TableOfContents } from '../../components/TableOfContents'
import type { CourseAppProps } from '../catalog'
...
function readPathFromHash(): string {
  const route = parseCourseRoute(window.location.hash)
  const path = route.kind === 'course' && route.courseId === 'ai-atlas' ? route.documentPath : undefined
  return path && documents.some((document) => document.path === path) ? path : homeDocument.path
}
...
function AiAtlasApp({ onExitCourse }: CourseAppProps) {
  ...
  const professorAudios = useMemo(() => getProfessorAudios(documents), [])
  ...
  const navigate = useCallback((path: string, anchor?: string, options: NavigationOptions = {}) => {
    const hash = courseDocumentHash('ai-atlas', path, anchor)
    const revealTarget = () => {
      globalThis.document.querySelector('.podcast-cue-active')?.classList.remove('podcast-cue-active')
      if (anchor) {
        const target = globalThis.document.getElementById(anchor)
        if (options.guided) target?.classList.add('podcast-cue-active')
        target?.scrollIntoView({ behavior: reduceMotion ? 'auto' : 'smooth', block: 'start' })
      } else {
        articleScrollRef.current?.scrollTo({ top: 0, behavior: reduceMotion ? 'auto' : 'smooth' })
      }
    }
    if (options.replace) {
      window.history.replaceState(null, '', hash)
      setCurrentPath(path)
      window.setTimeout(revealTarget, path === currentPath ? 0 : 180)
      ...
      <div className={`app-shell ${stageMode ? 'is-stage-mode' : ''}`}>
        ...
        <header className="topbar">
          ...
          <div className="topbar-actions">
            <button className="course-switch" type="button" onClick={onExitCourse} aria-label="Volver a todos los cursos" title="Todos los cursos">
              <SquaresFour size={18} />
              <span>Cursos</span>
            </button>
        ...
        <PodcastGuide
          audios={professorAudios}
          documents={documents}
          contextCategoryId={current.categoryId}
          stageMode={stageMode}
